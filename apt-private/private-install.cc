...
static void WriteApportReport(pkgCacheFile &Cache, std::string &title, std::vector<std::string> const &errors, int mode, bool distUpgrade)
{
   ...
   FileFd edspDump(flCombine(_config->FindDir("Dir::Log"), "edsp.log"),
   ...
   std::ofstream crash("/var/crash/apt-edsp.0.crash", std::ios::out | std::ios::trunc);
   chmod("/var/crash/apt-edsp.0.crash", 0600);
   ...
   crash << "ProblemType: AptSolver\n"
	 ...
	 << "Package: apt " << Cache->FindPkg("apt").CurrentVer().VerStr() << "\n"
	 << "Title: " << title << "\n"
	 << "SourcePackage: apt\n";
					...
					std::set<std::string> &UnknownPackages, APT::PackageVector &HeldBackPackages)
{
...

  {
	...
	return true;
     ...
     if (_config->Find("APT::Solver", "internal") == "internal" && (not res || (not Cache->FindPkg("apport").end() && Cache->FindPkg("apport")->CurrentVer)))
     {
